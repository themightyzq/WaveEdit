...
/**
 * Dialog for applying fade-in effect to selection.
 *
 * Features:
 * - Linear fade from 0% to 100% amplitude
 ...
 */
class FadeInDialog : public juce::Component
{
public:
    /**
     * Constructor.
     ...
     */
    FadeInDialog(AudioEngine* audioEngine,
                 AudioBufferManager* bufferManager,
                 int64_t selectionStart,
                 int64_t selectionEnd);
    ~FadeInDialog() override;
    ...
    /**
     * Get the currently selected fade curve type.
     * @return Selected curve type enum value (LINEAR if invalid)
     */
    FadeCurveType getSelectedCurveType() const
    {
        int selectedId = m_curveTypeBox.getSelectedId();
        if (selectedId < 1 || selectedId > 4)
        {
            // Fallback to LINEAR if invalid selection
            return FadeCurveType::LINEAR;
        }
        return static_cast<FadeCurveType>(selectedId - 1);
    }
...
private:
     ...
     * Creates a faded copy of the selection and plays it using preview system.
     */
    ...
    /**
     * Curve type ComboBox callback.
     * Saves preference and re-triggers preview if currently playing.
     */
    ...
    FadeCurvePreview m_curvePreview { true };  // true = fade in mode
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(FadeInDialog)
};
